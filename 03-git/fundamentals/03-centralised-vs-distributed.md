...
Before Git became the industry standard, many teams used **centralised version control systems** such as **SVN (Subversion)**.

In a centralised system, there is **one central server that stores the entire repository**. Developers check out files from that server, make changes locally, and then commit those changes back to it.

This server acts as the **single source of truth** for the project.

While this model worked for many years, it introduced several limitations for teams working on large or fast-moving projects.

Git introduced a **distributed version control model**, where every developer has a **complete copy of the repository**, including its full history.

This fundamentally changed how teams collaborate.
